...
from momex import Message, MomexConfig, SessionInfo, ShortTermMemory, StorageConfig
...
class TestShortTermMemory:
    ...
    def test_init_creates_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stm = ShortTermMemory("test:user", config)
    ...
    def test_init_with_session_id(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config = MomexConfig(storage=StorageConfig(path=tmpdir))
            stm = ShortTermMemory("test:user", config, session_id="my-session-123")
    ...
    def test_max_messages_limit(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config = MomexConfig(storage=StorageConfig(path=tmpdir))
            stm = ShortTermMemory("test:user", config, max_messages=5)
            for i in range(10):
                stm.add(f"Message {i}", role="user")
...
class TestShortTermMemoryPersistence:
    ...
    def test_persistence_across_instances(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stm1 = ShortTermMemory("test:user", config)
            ...
            stm2 = ShortTermMemory("test:user", config, session_id=session_id)
    ...
    def test_cleanup_expired(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stm = ShortTermMemory("test:user", config, session_ttl_hours=0)
    ...
    def test_db_path_hierarchical(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config = MomexConfig(storage=StorageConfig(path=tmpdir))
            stm = ShortTermMemory("org:team:user", config)
