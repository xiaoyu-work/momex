...
from momex.memory import _decode_deleted_ids, _encode_deleted_ids


class TestEncoding:
    def test_empty(self):
        assert _encode_deleted_ids(set()) == []

    def test_isolated_ids_stay_scalars(self):
        assert _encode_deleted_ids({1, 5, 9}) == [1, 5, 9]

    def test_consecutive_run_becomes_a_range(self):
        assert _encode_deleted_ids({1, 2, 3, 4}) == [[1, 4]]

    def test_mixed_runs_and_singletons(self):
        assert _encode_deleted_ids({1, 2, 3, 7, 10, 11}) == [[1, 3], 7, [10, 11]]
    ...
    def test_runs_compress_the_payload(self):
        ...
        encoded = len(json.dumps(_encode_deleted_ids(ids)))
...
class TestRoundTrip:
    ...
    def test_round_trip(self, ids):
        encoded = json.dumps(_encode_deleted_ids(ids))
        assert _decode_deleted_ids(json.loads(encoded)) == ids


class TestBackwardCompatibility:
    def test_reads_legacy_flat_list(self):
        assert _decode_deleted_ids([1, 2, 3]) == {1, 2, 3}

    def test_reads_legacy_string_digits(self):
        assert _decode_deleted_ids(["4", "5"]) == {4, 5}

    def test_reads_mixed_legacy_and_range(self):
        assert _decode_deleted_ids([1, [3, 5], "7"]) == {1, 3, 4, 5, 7}


class TestMalformedInput:
    def test_non_list_yields_empty(self):
        assert _decode_deleted_ids({"a": 1}) == set()
        assert _decode_deleted_ids(None) == set()

    def test_bad_entries_are_skipped(self):
        assert _decode_deleted_ids([1, "abc", None, [1, 2, 3], {}, [5, 6]]) == {
            1,
            5,
            6,
        }
    ...
    def test_inverted_range_skipped(self):
        assert _decode_deleted_ids([[9, 2]]) == set()

    def test_absurd_range_skipped(self):
        assert _decode_deleted_ids([[0, 10**12]]) == set()

    def test_booleans_are_not_treated_as_ints(self):
        assert _decode_deleted_ids([True, False]) == set()
